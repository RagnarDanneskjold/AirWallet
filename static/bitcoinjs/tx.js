...
var TX = new function () {
    ...
    this.init = function(eckey) {
        outputs = [];
		eckeys = {};
		eckeys[eckey.getBitcoinAddress()] = eckey;
    }

    this.initMultiple = function(_eckeys) {
        outputs = [];
		eckeys = {};
		for (var i in _eckeys) {
		  var eckey = _eckeys[i];
          eckeys[eckey.getBitcoinAddress()] = eckey;
		}
    }
    ...
    this.parseInputsBKC = function(data) {
        try {
            var res = tx_parseBKC(data);
        } catch(err) {
            var res = { "balance":"0" };
        }
    ...
    this.construct = function() {
        ...
        for (var hash in inputs) {
            ...
            for (var index in inputs[hash]) {
                ...
                var txin = new Bitcoin.TransactionIn({outpoint: {hash: b64hash, index: index, address: inputs[hash][index].address}, script: script, sequence: 4294967295});
        ...
        for (var i in outputs) {
            ...
            var fval = outputs[i].value;
            var value = new BigInteger('' + fval, 10);
        ...
        for (var i = 0; i < sendTx.ins.length; i++) {
			...
			var addr = selectedOuts[i].outpoint.address;
			if (!eckeys.hasOwnProperty(addr))
			  throw 'No matching private key';
			var eckey = eckeys[addr];
...
function tx_parseBCI(data, address) {
    ...
    for (var i in txs) {
        ...
        unspenttxs[lilendHash][o.tx_output_n] = {amount: value, script: script, address: address};
        balance = balance.add(value);
    }
    return {balance:balance, unspenttxs:unspenttxs};
}

// bkchain.org parser (adapted)
function tx_parseBKC(txs) {
    delete unspenttxs;
    var unspenttxs = {};
    var balance = BigInteger.ZERO;
    for (var i in txs) {
        ...
        var o = txs[i].output;
        var lilendHash = endian(o.tx);

        //convert script back to BBE-compatible text
        var script = dumpScript( new Bitcoin.Script(Crypto.util.hexToBytes(o.script)) );

        var value = new BigInteger('' + o.v, 10);
        if (!(lilendHash in unspenttxs))
            unspenttxs[lilendHash] = {};
        unspenttxs[lilendHash][o.txi] = {amount: value, script: script, address: txs[i].address};
        balance = balance.add(value);
    }
    return {balance:balance, unspenttxs:unspenttxs};
}
